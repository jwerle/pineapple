...
model.Model = function Model(name, schema, options) {
  ...
  this.toExposed = function(excluded) {
    ...
    if (this.exposed.length) {
      for (i = 0; i < this.exposed.length; i++) {
        ...
        exposed = pineapple.utils.setValueFromPath(key, value, json);
      }
      ...
      for (i = 0; i < excluded.length; i++) {
        if (excluded[i] in exposed) {
          delete exposed[excluded[i]]
        }
      }
...
model.Model.prototype.hide = function(hidee) {
